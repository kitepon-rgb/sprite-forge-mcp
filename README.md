...
- ✅ 素体生成（SDXL/Illustrious + BiRefNet matte）、ダメージ版編集（Qwen-Image-Edit・pose-lock）、画風LoRA v2、ControlNet 参照ポーズ。
...
詳細な実装現状・設計判断は **[CLAUDE.md](CLAUDE.md)**（実装の正典）を参照。
...
## ドキュメント
実装の最新は [CLAUDE.md](CLAUDE.md)。以下 docs/ は設計の正典（一部は実装で進化＝各冒頭の状態注記参照）。
